// ================================================================
// Sprint D-5 — Public unsubscribe / preference-change page.
// Reached from the morning-brief email footer; the JWT in the URL
// IS the credential (no login required).
// ================================================================
import { useEffect, useMemo, useState } from 'react';
import { Sunrise, Pause, CalendarRange, MailX, CheckCircle2, Loader2 } from 'lucide-react';
...
import { getUnsubInfo, applyUnsub } from '@/api/v2/morningBrief';

const LABEL = {
  appName:      { tr: 'Zyrix FinSuite',          en: 'Zyrix FinSuite',           ar: 'زيريكس فينسوت' },
  pageTitle:    { tr: 'Sabah Brifingi',           en: 'Morning Brief',           ar: 'إيجاز الصباح' },
  prompt:       { tr: 'Sabah brifingini almak istemiyor musun?',
                  en: 'Stop receiving the morning brief?',
                  ar: 'هل تريد إيقاف إيجاز الصباح؟' },
  ...
  reasons: {
    tooOften:   { tr: 'Çok sık',                  en: 'Too frequent',             ar: 'متكرر جدًا' },
  ...
  weekly:       { tr: 'Haftalık gönder',           en: 'Send weekly instead',      ar: 'إرساله أسبوعيًا' },
  ...
  donePause:    { tr: 'Brifing 30 gün duraklatıldı.', en: 'Brief paused for 30 days.', ar: 'تم إيقاف الإيجاز 30 يومًا.' },
  doneWeekly:   { tr: 'Artık haftada bir gönderilecek.', en: 'Now sending weekly.', ar: 'سيتم الإرسال أسبوعيًا الآن.' },
  doneCta:      { tr: 'Panele dön',               en: 'Back to dashboard',         ar: 'العودة إلى اللوحة' },
  loading:      { tr: 'Yükleniyor…',               en: 'Loading…',                  ar: 'جارٍ التحميل…' },
  invalid:      { tr: 'Bağlantı geçersiz veya süresi dolmuş. Lütfen yeni e-postanı bekle.',
                  en: 'This link is invalid or has expired. Please wait for the next email.',
                  ar: 'هذا الرابط غير صالح أو منتهي الصلاحية.' }
};
// Translator: supports both top-level keys ("prompt") and one-level
// dotted keys ("reasons.tooOften") so the LABEL tree can stay nested.
function _(k, lang) {
  if (k.includes('.')) {
    const [a, b] = k.split('.');
...
const REASON_KEYS = ['tooOften', 'notRelevant', 'overload', 'other'];
    ...
    <div dir={dir} style={{
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 540 }}>
        ...
        <header style={{ textAlign: 'center', marginBottom: SPACE.lg }}>
          <NeonBadge tone="amber" size="sm" leading={<Sunrise size={11} />}>
          ...
          {!loading && !error && action && (
            <div style={{ padding: SPACE.lg, textAlign: 'center' }}>
              ...
              <p style={{ margin: `0 0 ${SPACE.lg}px`, color: CINEMATIC.text.pearlDim, fontSize: 13 }}>
                {info?.merchant?.emailMasked ? `${info.merchant.emailMasked}` : ''}
              </p>
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <ActionButton
                  ...
                  label={_('weekly', language)}
                  onClick={() => apply('weekly')}
                  disabled={pending !== null}
                  loading={pending === 'weekly'}
                  tone="violet"
                />
                <ActionButton
                  icon={<MailX size={13} />}
